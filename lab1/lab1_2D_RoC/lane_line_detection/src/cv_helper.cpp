...
void subplot(std::string title, std::vector<cv::Mat> &v, unsigned int w, unsigned int h) {
    if (w * h < v.size()) {
        throw std::runtime_error("Grid size is too small for number of images. Please choose appropriate numbers for w and h.\n");
    }
    ...
    cv::Mat DispImage = cv::Mat::zeros(cv::Size((int)((x / scale) * w + 2 * step + (w - 1) * step),
                                       (int)((y / scale) * h + 2 * step + (h - 1) * step)), type);
    ...
    for (i = 0, m = step, n = step; i < nArgs; i++, m += (step + (int)(x / scale))) {
        ...
        // If it is NULL, release the image, and return
        if (img.empty()) {
            throw std::invalid_argument("Invalid Image. Image is empty.\n");
        }
        /*
...
void HSVmanager::setHSVImage(cv::Mat &img)
{
