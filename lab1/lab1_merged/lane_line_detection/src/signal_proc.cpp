...
void polyFit(const cv::Mat &src_x, const cv::Mat &src_y, cv::Mat &dst, int order) {
    if (src_x.cols != 1 && src_x.channels() != 1 && src_y.cols != 1 && src_y.channels() != 1) {
        throw std::invalid_argument("Dimensions of inputs (src_x, src_y) are invalid. Expected dimensions are of the form [1 n 1] (width, height, channels).\n");
    }
    else if (src_x.type() != 6 || src_y.type() != 6) {
        throw std::invalid_argument("Input types are invalid. src_x and src_y must be CV_64F.\n");
    }
...
void polyVal(const cv::Mat& src, cv::Mat& dst, const cv::Mat& best_fit) {
    if (src.cols != 1 && src.channels() != 1 && best_fit.cols != 1 && best_fit.channels() != 1) {
        throw std::invalid_argument("Dimensions of inputs (src, best_fit) are invalid. Expected dimensions are of the form [1 n 1] (width, height, channels).\n");
    }
    else if (src.type() != 6 || best_fit.type() != 6) {
        throw std::invalid_argument("Input types are invalid. src and best_fit must be CV_64F.\n");
    }
    /*
    cv::Mat temp;
